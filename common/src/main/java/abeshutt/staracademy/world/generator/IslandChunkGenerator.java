package abeshutt.staracademy.world.generator;
...
public class IslandChunkGenerator extends ChunkGenerator {
    ...
    public static final MapCodec<IslandChunkGenerator> CODEC = RecordCodecBuilder.mapCodec(
            instance -> instance.group(FlatChunkGeneratorConfig.CODEC.fieldOf("settings").forGetter(IslandChunkGenerator::getConfig))
                    .apply(instance, instance.stable(IslandChunkGenerator::new))
    );

    private final FlatChunkGeneratorConfig config;

    public IslandChunkGenerator(FlatChunkGeneratorConfig config) {
        super(new FixedBiomeSource(config.getBiome()),
                Util.memoize(config::createGenerationSettings));
        this.config = config;
    }

    @Override
    public StructurePlacementCalculator createStructurePlacementCalculator(RegistryWrapper<StructureSet> structureSetRegistry,
